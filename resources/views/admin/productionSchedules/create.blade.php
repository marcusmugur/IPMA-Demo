        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.productionSchedule.title_singular') }}
                </div>
                <div class="panel-body">

                    <form action="{{ route("admin.production-schedules.store") }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">                      
                        ...
                        <div class="form-group {{ $errors->has('room') ? 'has-error' : '' }}">
                            ...
                            <select id="room" name="room" class="form-control" required>
                                ...
                                @foreach(App\ProductionSchedule::ROOM_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('room', 'room 1') === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('room'))
                                <p class="help-block">
